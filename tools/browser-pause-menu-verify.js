...
const keys={Space:[' ',32],KeyW:['w',87],KeyA:['a',65],KeyS:['s',83],KeyD:['d',68],ArrowRight:['ArrowRight',39],Escape:['Escape',27]};
...
async function sim(ev){return ev(`(()=>({started:__started(),paused:__paused(),time:__gameTime(),x:__P.pos.x,y:__P.pos.y,z:__P.pos.z,level:__LEVEL()&&__LEVEL().id,won:!!__W.won,picker:getComputedStyle(document.getElementById('start')).display,win:getComputedStyle(document.getElementById('win')).display,playing:document.body.classList.contains('playing'),pausedClass:document.body.classList.contains('paused'),pauseBtn:getComputedStyle(document.getElementById('pauseBtn')).display,input:__INPUT_STATE(),camel:!!__P.camel,sled:!!__P.sled,moveZone:__P.moveZone,spaceThrust:!!__P.spaceThrust}))()`);}
...
async function levelLifecycle(cdp,index,result){
  ...
  await mouseTap(cdp,cdp.evaluate,'pauseResume');await waitEval(cdp.evaluate,`!__paused()`,3000);const r0=await sim(cdp.evaluate);await holdKey(cdp,'KeyD',360);const r1=await sim(cdp.evaluate);assert(r1.time>r0.time&&moved(r0,r1)>0.06,`${label}: gameplay did not resume`);assert(!r1.won,`${label}: completion occurred during lifecycle`);
  ...
  result.levels.push({level:levelId,status:'PASS',active:'movement+gameTime',freeze:'player position+gameTime',pausedInput:'move+jump blocked',resume:'movement+gameTime',mainMenu:'clean picker/no win',subsequentRun:'clean'});
}
...
async function spaceTransient(cdp,result){
  const label='Level 4 open-space thrust';await fresh(cdp);await pickerTo(cdp,3);await waitEval(cdp.evaluate,`__started()&&__LEVEL().id==='level4'`,7000);
...
async function camelTransient(cdp,result){
  const label='Level 5 mounted camel';await fresh(cdp);await pickerTo(cdp,4);await waitEval(cdp.evaluate,`__started()&&__LEVEL().id==='level5'&&__W.camels.length>0`,7000);const camel=await cdp.evaluate(`(()=>{const c=__W.camels[0];return {x:c.x,z:c.z};})()`);await driveTo(cdp,cdp.evaluate,camel.x,camel.z,label,24000);await tapKey(cdp,'Space',60);await waitEval(cdp.evaluate,`!!__P.camel`,3500);await holdKey(cdp,'KeyW',350);await tapKey(cdp,'Escape');await waitEval(cdp.evaluate,`__paused()`,3000);
