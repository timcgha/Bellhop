...
async function sledTransient(cdp,result){
  const label='Level 6 sliding sled';await fresh(cdp);await startPickerCard(cdp,5,'level6',label);await waitEval(cdp.evaluate,`!!(window.__WINTER&&window.__WINTER.sled)`,7000);const sled=await cdp.evaluate(`({x:window.__WINTER.sled.x,z:window.__WINTER.sled.z})`);await driveTo(cdp,cdp.evaluate,-5,-174,label+' hilltop route',30000);await driveTo(cdp,cdp.evaluate,sled.x,sled.z+0.65,label,30000);await waitEval(cdp.evaluate,`__P.pos.y>5.2&&Math.hypot(__P.pos.x-__WINTER.sled.x,__P.pos.z-__WINTER.sled.z)<2.25`,6000);await tapKey(cdp,'Space',70);await waitEval(cdp.evaluate,`!!__P.sled&&__WINTER.sled.phase==='sliding'`,3500);await waitEval(cdp.evaluate,`__WINTER.sled.progress>0.04`,4000);await tapKey(cdp,'Escape');await waitEval(cdp.evaluate,`__paused()`,3000);
