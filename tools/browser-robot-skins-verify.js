#!/usr/bin/env node
// BH-002: real UI selection, shared-material safety, rendering, persistence,
// and a non-Classic robot through all six real picker lifecycles.
...
async function key(c,code,down){const [k,v]=keyMap[code];await c.send('Input.dispatchKeyEvent',{type:down?'keyDown':'keyUp',key:k,code,windowsVirtualKeyCode:v,nativeVirtualKeyCode:v,text:down&&k.length===1?k:undefined});}
async function tapKey(c,code,ms=60){await key(c,code,true);await sleep(ms);await key(c,code,false);await sleep(100);}
...
async function drive(c,tx,tz,label,timeout=50000){
  const t=Date.now();let last=null,stuck=0;
  while(Date.now()-t<timeout){
    await c.ev('(()=>{__CAM.yaw=0;__CAM.lastManual=1e9;})()');const s=await sim(c);assert(!s.won,'unexpected completion on '+label);if(s.dead){await sleep(450);continue;}
...
async function transients(c,index,result){
  if(index===3){await key(c,'Space',true);await wait(c,`__P.moveZone==='openSpace'&&__P.spaceThrust`,3500);await sleep(180);await appearance(c,'purple');await c.shot('purple-space-thrust');await key(c,'Space',false);result.transients.push({level:4,state:'real open-space thrust',status:'PASS'});}
  if(index===4){const q=await c.ev(`({x:__W.camels[0].x,z:__W.camels[0].z})`);await drive(c,q.x,q.z,'camel',24000);await tapKey(c,'Space');await wait(c,'!!__P.camel',3500);await hold(c,['KeyW'],300);await appearance(c,'purple');assert(await c.ev(`Math.abs(__PLAYER().position.x-__P.pos.x)<.02&&__PLAYER().position.y-__P.pos.y>1.45`),'camel attachment');await c.shot('purple-mounted-camel');result.transients.push({level:5,state:'real camel mount/movement',status:'PASS'});}
  if(index===5){
    ...
    const q=await c.ev(`({x:__WINTER.sled.x,z:__WINTER.sled.z})`);await drive(c,q.x,q.z+.65,'sled',12000);await wait(c,`__P.pos.y>5.2&&Math.hypot(__P.pos.x-__WINTER.sled.x,__P.pos.z-__WINTER.sled.z)<2.25`,6000);await tapKey(c,'Space',70);await wait(c,`!!__P.sled&&__WINTER.sled.phase==='sliding'&&__WINTER.sled.progress>.04`,4000);await appearance(c,'purple');await c.shot('purple-sliding-sled');result.transients.push({level:6,state:'real Snoozle progression/mounted sliding sled',status:'PASS'});
  }
}
async function selector(c,w,h,result){
  const touch=w!==1280;await viewport(c,w,h,touch);await tap(c,'lvl2',touch);assert(!await c.ev('__started()'),'level selection launched unexpectedly');const original=(await skins(c)).equipped,idx=await c.ev('__pickerIdx()'),roots=await sceneRoots(c),world=await nonRobotMaterials(c);
  await open(c,touch);await checkPreview(c,original,original);await tap(c,'skin-yellow',touch);await checkPreview(c,'yellow',original);
  ...
  await tapKey(c,'ArrowRight');await tapKey(c,'Space');assert(await c.ev(`!__started()&&__pickerIdx()===${idx}`),'panel input activated underlying picker');
...
async function main(){
  ...
  try{
    ...
    for(const [w,h] of [[1280,720],[390,844]]){
      ...
      for(let i=0;i<6;i++){
        console.log('SKINS lifecycle '+w+'x'+h+' level '+(i+1));await start(c,i,w!==1280);await appearance(c,'purple');const active=await movement(c);
        ...
        await menu(c,w!==1280);await appearance(c,'purple');const isolation=await audit(c,baseline);assertAudit(isolation);
        ...
        result.levels.push({viewport:w+'x'+h,level:i+1,status:'PASS',skin:'purple',active,isolation,conch:conch?{registered:true,detached:true}:null});
      }
      await start(c,0,w!==1280);await appearance(c,'purple');await movement(c);await menu(c,w!==1280);assertAudit(await audit(c,baseline));
    }
