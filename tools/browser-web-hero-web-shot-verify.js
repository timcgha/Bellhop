...
async function connect(){
  ...
  const shot=async name=>{const r=await send('Page.captureScreenshot',{format:'png'}),b=Buffer.from(r.data,'base64'),file=name+'.png';fs.writeFileSync(path.join(out,file),b);const row={file,bytes:b.length,pixelWidth:b.readUInt32BE(16),pixelHeight:b.readUInt32BE(20),sha256:createHash('sha256').update(b).digest('hex')};report.screenshots.push(row);return row;};
...
async function captureFamily(c,family,input,label,screenshot){
  ...
  if(screenshot)await c.shot(screenshot);
  ...
  const row={family,input,level:await c.ev('__LEVEL().id'),positioning:'explicit evidence fixture only; shipped input/projectile/collision/capture/defeat paths remain real',targetIndex:target.index,wrapPixels:pixels,eventTail:done.events.slice(-4)};report.levelMatrix.push(row);return row;
}
...
async function main(){
  ...
  try{
    ...
    await scenario('ordinary Meadow keyboard journey',async()=>{await key(c,'KeyW',true);try{await wait(c,"Math.hypot(__P.pos.x-__W.gloops[0].x,__P.pos.z-__W.gloops[0].z)<13",6000);}finally{await key(c,'KeyW',false);}await tapKey(c,'KeyX');await wait(c,"__WEB_SHOT.snapshot().events.some(e=>e.type==='wrapped'&&e.family==='gloop')",3000);return {method:'real menu start, continuous KeyW traversal through entrance, then KeyX; no position assignment',events:await c.ev('__WEB_SHOT.snapshot().events')};});
