...
function domPass(dom,reduced){
  ...
  if(dom.styles.title.color!=='rgb(21, 94, 100)'||dom.styles.title.textShadow!=='none'||dom.styles.title.textStroke)return false;
...
async function candidateRun(cdp,result){
  ...
  await navigate(cdp,844,390,false,false);await startLevel(cdp,0);await installFanfareCounter(cdp);await wakeAll(cdp);await waitEval(cdp,'__W.won',5000);const autoStart=Date.now();await waitEval(cdp,clean,20500);result.returns.auto={clean:true,elapsedMs:Date.now()-autoStart,input:await cdp.evaluate('__INPUT_STATE()')};
