...
function stageConchWinPose(){
  // Stand inside the hollow chamber looking out the mouth so the player stays visible.
  if(!CONCH)return;
  const standZ=CONCH.chamberZ!=null?CONCH.chamberZ:(CONCH.cz+1.0);
  P.pos.set(CONCH.cx,0.45,standZ);
  P.vel.set(0,0,0);P.yaw=0;P.grounded=true;P.puff=true;P.puffAir=0;endHover();
  CAM.look.x=CONCH.cx;CAM.look.y=1.6;CAM.look.z=standZ+0.4;
  CAM.pos.x=CONCH.cx;CAM.pos.y=3.2;CAM.pos.z=CONCH.doorZ+6.5;
  CAM.yaw=0;CAM.pitch=0.22;CAM.fovKick=Math.max(CAM.fovKick,8);CAM.shake=Math.max(CAM.shake,0.35);
}
...
function updateConchChamberFade(dt){
  ...
  const inside=inConchInterior();
  const target=inside?0.16:1;
  if(CONCH.chamberFade==null)CONCH.chamberFade=1;
  CONCH.chamberFade=damp(CONCH.chamberFade,target,8,dt);
...
function updateConch(dt,winT){
  ...
  if(CONCH.interiorGlow)CONCH.interiorGlow.material.opacity=CONCH.open?(0.38+Math.min(CONCH.openT,1)*0.45):0.05;
  if(CONCH.openMouth&&CONCH.open)CONCH.openMouth.material.opacity=0.72+Math.sin(time*2.2)*0.12;
  if(CONCH.openRim&&CONCH.open)CONCH.openRim.material.opacity=0.48+Math.sin(time*1.6)*0.12;
  ...
  if(CONCH.openCavity&&CONCH.open){
    const k=CONCH.openCavity.material;
    if(k&&k.opacity!=null)k.opacity=0.35+Math.sin(time*1.5)*0.08;
  }
  if(winT>=0){
    ...
    if(CONCH.interiorGlow)CONCH.interiorGlow.material.opacity=0.6+Math.sin(time*2.4)*0.12;
    if(CONCH.openMouth)CONCH.openMouth.material.opacity=0.85+Math.sin(time*3)*0.1;
    if(CONCH.openRim)CONCH.openRim.material.opacity=0.6+Math.sin(time*2.5)*0.12;
...
function buildConch(cx,cz){
  ...
  // Mouth lip rings the hollow; does not fill it
  g.add(mesh(SPH,lip,-2.15,2.1,DOOR_Z-0.15,1.35,2.3,1.1));
  g.add(mesh(SPH,lip,2.35,2.15,DOOR_Z-0.2,1.4,2.35,1.15));
  g.add(mesh(SPH,lip,0.1,4.05,DOOR_Z-0.05,2.8,0.9,1.0));
  g.add(mesh(SPH,lip,0.1,0.45,DOOR_Z+0.15,2.6,0.55,0.95));
  for(let i=0;i<5;i++){
    const a=-0.9+i*0.45,cy=2.1+Math.sin(a)*1.3,cxo=Math.cos(a)*1.95;
    g.add(mesh(SPH,rib,cxo,cy,DOOR_Z-0.05,0.42,0.36,0.38));
  }
  const mouthFill=mesh(SPH,inner,0.1,2.1,DOOR_Z-0.35,DOOR_W*0.48,DOOR_H*0.46,0.95);g.add(mouthFill);CONCH.mouthFill=mouthFill;
  ...
  const openCavity=mesh(SPH,new THREE.MeshBasicMaterial({color:0xfff0d0,transparent:true,opacity:0.28,depthWrite:false}),0,2.0,chMidZ,CH_HALF_W*0.85,CH_H*0.42,chDepth*0.35);
  openCavity.visible=false;chamber.add(openCavity);CONCH.openCavity=openCavity;
  ...
  const openMouth=mesh(SPH,new THREE.MeshBasicMaterial({color:0xfff8d0,transparent:true,opacity:0.82,depthWrite:false}),0,DOOR_Y+DOOR_H*0.5,DOOR_Z+0.2,DOOR_W*0.7,DOOR_H*0.64,0.7);
  openMouth.visible=false;g.add(openMouth);CONCH.openMouth=openMouth;
  const openRim=mesh(SPH,new THREE.MeshBasicMaterial({color:0xffe39a,transparent:true,opacity:0.55,depthWrite:false}),0,DOOR_Y+DOOR_H*0.5,DOOR_Z+0.45,DOOR_W*0.92,DOOR_H*0.82,0.35);
  openRim.visible=false;g.add(openRim);CONCH.openRim=openRim;
  const openHalo=mesh(SPH,new THREE.MeshBasicMaterial({color:0xc8f0ff,transparent:true,opacity:0.3,depthWrite:false}),0,DOOR_Y+DOOR_H*0.5,DOOR_Z+0.85,DOOR_W*1.15,DOOR_H*1.0,0.4);
  openHalo.visible=false;g.add(openHalo);CONCH.openHalo=openHalo;
  ...
  interiorGlow.position.set(0,2.0,chMidZ);interiorGlow.scale.set(2.2,1.9,chDepth*0.4);chamber.add(interiorGlow);CONCH.interiorGlow=interiorGlow;
