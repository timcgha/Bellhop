...
function pollGamepad(dt){
  ...
  if(edge(3))IN.y=true;
  if(!started&&b.some(x=>x))startGame();
  GP.prev=b;
}
...
ctl.addEventListener('pointerdown',e=>{
  if(!started)startGame();initAudio();
...
function bindBtn(id,down,up){const el=$(id);el.addEventListener('pointerdown',e=>{e.stopPropagation();e.preventDefault();if(!started)startGame();initAudio();if(el.setPointerCapture)el.setPointerCapture(e.pointerId);down();});const u=e=>{e.stopPropagation();if(up)up();};el.addEventListener('pointerup',u);el.addEventListener('pointercancel',u);el.addEventListener('lostpointercapture',u);}
