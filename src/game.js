...
function updateCamera(dt){
  ...
  const pitchMin=inWreck?0.22:0.08;
  ...
  const tx=P.pos.x,ty=P.pos.y+(inWreck?1.35:1.1),tz=P.pos.z;
  const cp=Math.cos(CAM.pitch),spp=Math.sin(CAM.pitch);
  const dist=inWreck?10.8:8.5;
  const dx=Math.sin(CAM.yaw)*cp*dist,dy=spp*dist,dz=Math.cos(CAM.yaw)*cp*dist;
  let k=1;for(let i=1;i<=8;i++){const f=i/8;if(insideSolid(tx+dx*f,ty+dy*f,tz+dz*f,inWreck?0.2:0.35)){k=(i-1)/8;break;}}k=Math.max(k,inWreck?0.45:0.25);
