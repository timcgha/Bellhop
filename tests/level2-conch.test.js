// Stage 6B: The Conch — closed/open states, Snoozle homes, Level 2 win.
...
function crossesWreckHull(pts){
  // representative solid Wreck volumes: side walls and stern wall (not decorative spars)
  ...
  return pts.some(p=>{
    ...
    // treat the open interior shaft as non-crossing; fail only if near outer wall shells
...
// ---- closed: cannot enter, cannot win ----
{
  const H=boot();startL2(H);
  const c=H.W.conch;
  wakeAllButLast(H);
  ok(!c.open,'Conch stays closed with fewer than four Snoozles awake');
  ok(!H.W.won,'reaching the Conch with three awake does not win');
  const before=push(H,0,2.0,c.doorZ+3,0,-4,140);
  ok(before.z>c.doorZ-0.2,'closed doorway physically blocks entry');
  H.P.pos.set(c.trigger.x,c.trigger.y,c.trigger.z);H.P.vel.set(0,0,0);H.frames(30);
  ok(!H.W.won,'standing in the interior trigger while closed does not win');
  ok(H.el('win').style.display!=='flex','closed Conch never shows the win banner');
}

// ---- fourth wake opens Conch but does not win ----
{
  const H=boot();startL2(H);
  const c=H.W.conch;
  wakeAllButLast(H);
  ok(!H.W.conch.open,'still closed before the fourth wake');
  wake(H,H.W.snoozles[3]);H.frames(8);
  ok(H.W.conch.open,'fourth Snoozle wake opens the Conch');
  ok(H.W.solids.indexOf(H.W.conch.doorSolid)<0,'open doorway removes door collision');
  ...
  ok(!H.W.WM||!H.W.WM.party,'opening the Conch does not start windmill party');
}

// ---- four awake + outside = no win; enter open = win once ----
{
  const H=boot();startL2(H);
  ...
  // swim through the open door into the interior trigger
  push(H,0,2.0,H.W.conch.doorZ+2.5,0,-3.5,90);
  H.P.pos.set(H.W.conch.trigger.x,H.W.conch.trigger.y,H.W.conch.trigger.z);H.P.vel.set(0,0,0);
  H.frames(8);
  ok(H.W.won,'entering the open Conch trigger wins Level 2');
...
// ---- open door is passable; shell still blocks ----
{
  const H=boot();startL2(H);
  wakeAll(H);H.frames(6);
  const through=push(H,0,2.0,H.W.conch.doorZ+3,0,-4,160);
  ok(through.z<H.W.conch.doorZ-0.5,'open doorway lets the player swim inside');
  ok(canStand(H,0,H.W.conch.cz+1.2,50)||H.P.pos.z<H.W.conch.doorZ,
    'interior remains traversable with Level 2 physics');
  const blocked=push(H,0,2.0,H.W.conch.cz-8,0,4,100);
  ok(blocked.z<H.W.conch.cz-4.5,'major shell back surface still blocks');
}
...
// ---- full logical flow ----
{
  ...
  ok(H.W.won,'only entering the open Conch completes Level 2');
}
...
// ---- snoozle homes + wreck departure path ----
{
  ...
  // reconstruct the path: nest -> waypoints -> home, and ensure it does not clip hull shells
