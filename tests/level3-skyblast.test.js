// Level 3 Stage 1: Sky Blast state model, long-leap carry, production teaching gap.
...
const teach=L.teachGaps.find(g=>g.id==='teachPower')||L.teachGaps[1];
const gapNear=teach.nearLipZ;
const gapFar=teach.farEdgeZ;
...
function grabSky(){
  ...
  if(c){P.pos.set(c.x,c.y+1.3,c.z);P.vel.set(0,-10,0);P.slam=2;P.grounded=false;frames(12);}
  const w=W.powers.find(p=>p.kind==='sky'&&!p.got);
  if(w){P.pos.set(w.x,w.y-0.5,w.z);P.vel.set(0,0,0);frames(8);}
...
function leapOnce(opts){
  ...
  // Measure carry on the opening meadow along +X (no lava interruptions).
  settle(-6,0.4,16);
  P.yaw=Math.PI/2;
...
settle(0,0.4,18);P.hasSkyBlast=true;P.puff=true;P.leapBoost.set(0,0,0);P.yaw=Math.PI;
kd({code:'KeyA',preventDefault(){},repeat:false});
...
settle(-4,0.4,16);P.hasSkyBlast=true;P.puff=true;P.yaw=Math.PI/2;P.leapBoost.set(0,0,0);P.inv=99;
...
settle(-4,0.4,16);P.hasSkyBlast=true;P.puff=true;P.yaw=Math.PI/2;H.CAM.yaw=0;P.inv=99;
...
// Enemy knockback via ember (Level 3's projectile) — no gloop required on the production route.
settle(0,0.4,14);P.hasSkyBlast=true;P.puff=true;P.leapBoost.set(0,0,-10);P.inv=0;P.hp=4;
const hp0=P.hp;
...
let hurt=false;
for(let i=0;i<40;i++){
  emb.pos.set(P.pos.x,P.pos.y+0.5,P.pos.z);
  frames(1);
  if(P.hp<hp0){hurt=true;break;}
}
ok(hurt,'took an enemy/ember hit');
...
function attemptGap(powered){
  // Run toward -Z across the powered teaching gap.
  const startZ=gapNear+6;
  settle(0,6.9,startZ);P.yaw=Math.PI;P.hasSkyBlast=!!powered;P.puff=true;P.leapBoost.set(0,0,0);P.inv=99;
  ...
  for(let i=0;i<260;i++){
    ...
    if(P.pos.z<=gapFar+1.2&&P.pos.y>=6.3&&P.grounded){cleared=true;landZ=P.pos.z;landY=P.pos.y;break;}
...
ok(!unpowered.cleared,'unpowered movement cannot clear the teaching gap (land z='+unpowered.landZ.toFixed(2)+')');
ok(powered.cleared,'powered running leap clears the teaching gap (land z='+powered.landZ.toFixed(2)+')');
ok(powered.airTravel>unpowered.airTravel*1.05,'powered air travel exceeds unpowered ('+powered.airTravel.toFixed(2)+' vs '+unpowered.airTravel.toFixed(2)+')');

settle(-4,0.4,16);P.hasSkyBlast=true;P.puff=true;P.yaw=Math.PI/2;P.inv=99;
