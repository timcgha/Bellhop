// Level 3 Stage 3: Cinders, embers, Wisps, salamanders, geysers (production placements).
...
ok(notesAtLoad>=3,'counted notes exist at load ('+notesAtLoad+')');
...
ok(mid.size===1.0,'Cinder size/HP tiers match gloop-style small/mid/big');

settle(mid.x+1.3,mid.y||0.4,mid.z);faceToward(mid.x,mid.z);P.inv=99;
const hpC0=mid.hp;
tap('KeyK',2);frames(4);
...
H.test.loadLevel(2);
settle(0,0.4,14);P.inv=0;P.hp=4;P.hasSkyBlast=true;P.puff=true;P.leapBoost.set(0,0,-9);
const hpE=P.hp;
...
settle(0,0.4,14);P.hasSkyBlast=true;P.inv=0;P.hp=4;P.leapBoost.set(0,0,-4);
P.pos.set(side.x,0.2,side.z);P.vel.set(0,-1,0);P.grounded=false;frames(3);
...
settle(wisp.g.position.x,19.5,wisp.g.position.z);P.inv=0;P.hp=4;
...
settle(w2.g.position.x+1.2,19.5,w2.g.position.z);faceToward(w2.g.position.x,w2.g.position.z);P.inv=99;
tap('KeyK',3);frames(6);
ok(w2.alive,'spin does not defeat Wisp');
P.pos.set(w2.g.position.x,w2.g.position.y+0.8,w2.g.position.z);P.vel.set(0,-10,0);P.slam=2;P.grounded=false;frames(10);
ok(w2.alive,'slam does not defeat Wisp');

settle(w2.g.position.x+1.0,19.5,w2.g.position.z);faceToward(w2.g.position.x,w2.g.position.z);P.inv=99;P.gustCD=0;
tap('KeyJ',2);frames(4);
...
P.pos.set(nw2.g.position.x,19.5,nw2.g.position.z+1.15);P.vel.set(0,0,0);P.grounded=true;faceToward(nw2.g.position.x,nw2.g.position.z);P.inv=99;P.gustCD=0;frames(2);
tap('KeyJ',2);frames(6);
...
noteSal.speed=0;
P.pos.set(noteSal.x,0.4,noteSal.z+1.0);P.vel.set(0,0,0);P.grounded=true;
P.yaw=Math.atan2(noteSal.x-P.pos.x,noteSal.z-P.pos.z);P.inv=99;P.gustCD=0;P.slam=0;
kd({code:'KeyJ',preventDefault(){},repeat:false});frames(2);ku({code:'KeyJ'});frames(6);
...
P.gustCD=0;kd({code:'KeyJ',preventDefault(){},repeat:false});frames(2);ku({code:'KeyJ'});frames(4);
