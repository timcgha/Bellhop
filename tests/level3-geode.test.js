...
ok(!L.steps.some(s=>s[0]==='protoEndpoint'&&Math.abs(s[3]-(-368))<1),'Stage 5 temporary Climb placeholder endpoint removed');
ok(!insideSolidAt(0,22.5,-368,0.25),'Hollow exit is open into the Climb');
ok(R.climbBase&&R.endpoint.z<R.climbBase.z,'Climb continues past the Hollow exit');
...
ok(W.notes.length===10,'Level 3 counted-note total is 10 with Climb challenge notes');
...
// ---- Soft-return endpoint last (leaves started=false) — now at Climb rim ----
reloadL3();
const ep2=W.protoEndpoints[0];
ok(ep2&&Math.abs(ep2.z-R.endpoint.z)<1,'rim endpoint present');
...
ok(H.getLevel().mandatoryLeaps.filter(l=>!/^climb\d/.test(l.id)).length===5,'Lava Field mandatory leaps unchanged');
