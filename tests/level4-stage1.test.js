...
// ---- hold-A forever ----
{
  ...
  holdJump(H,2);
  let maxSp=0,gravFall=false,lostThrust=false;
  for(let i=0;i<900;i++){
    H.frames(1);
    if(H.P.pos.y<22||Math.hypot(H.P.pos.x,H.P.pos.z)>6){H.P.pos.set(0,28,0);H.P.vel.y=Math.max(H.P.vel.y,0);}
    const m=H.getMovement();
    if(m.zone!=='openSpace')lostThrust=true;
    if(!m.spaceThrust&&i>5)lostThrust=true;
    maxSp=Math.max(maxSp,m.speed);
    if(H.P.vel.y<-1.2&&m.zone==='openSpace')gravFall=true;
  }
  releaseJump(H);
